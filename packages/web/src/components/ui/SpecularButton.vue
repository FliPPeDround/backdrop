...
<template>
  <button
    ...
    :type="type"
    :disabled="disabled"
    class="leading-none tracking-[0.01em] font-medium m-0 outline-none rounded-[var(--sb-radius)] border-none inline-flex cursor-pointer shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] transition-transform duration-150 [backdrop-filter:blur(var(--sb-blur))] [background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] items-center justify-center relative focus-visible:outline-2 focus-visible:outline-offset-[3px] disabled:opacity-55 disabled:cursor-default active:scale-[0.97] disabled:active:scale-100" :class="[
      sizeClass,
    ]"
    :style="buttonStyle"
  >
    <span
      ref="fxRef"
      aria-hidden="true"
      class="pointer-events-none absolute z-1 [&_canvas]:h-full [&_canvas]:w-full [&_canvas]:block -inset-5"
    />
    <span class="relative z-2"><slot>Get Started</slot></span>
  </button>
</template>
